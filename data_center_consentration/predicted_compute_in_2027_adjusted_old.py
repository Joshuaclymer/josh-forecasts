...
# ######################### Make adjustment to compute distributions

def adjust_sample(sample, boost_factor): # boost factor of 3 means that people are 3x less likely to report a cluster 10x smaller than the largest one
    # first get the largest value
    max_value = sample.max()

    new_items = []
    # now for every other value, calculate a boost on that value based on how much smaller it is than the max
    for item in sample:
        new_items.append(item)
        boost = int(boost_factor ** np.log(max_value / item))
...
df.loc[df['Op_Year'].isin([2022, 2023, 2024, 2025]), 'Combined_Year'] = df['Op_Year']
df.loc[df['Planned_Year'].isin([2026, 2027]), 'Combined_Year'] = df['Planned_Year']
...
for year in years:
    if year <= 2025:
        ...
        sample = year_df['H100 equivalents']
        sample = adjust_sample(sample, ADJUST_FACTOR)
...
sample_2025 = year_2025_df['H100 equivalents']
sample_2025 = adjust_sample(sample_2025, ADJUST_FACTOR)
...
for year in [2026, 2027]:
    ...
    new_sample = []
    for item in sample_2025:
        new_sample.append(item * shift_factor)

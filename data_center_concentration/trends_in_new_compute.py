...
fig.suptitle('Proportion of Compute in Newly Built Clusters by Size', fontsize=12)
...
# Calculate average cluster size for each year up to 2025 (excluding 2026)
for i, year in enumerate(years):
    if year == 2026:
        continue  # Skip 2026
...
if len(year_indices_for_regression) > 1:
    ...
    # Calculate the average size for 2025
    ...
    # Calculate total compute for forecasted 2026
    total_compute_2026 = year_2026_df['H100 equivalents'].sum()
    ...
    for j, (min_size, max_size) in enumerate(size_categories):
        ...
        category_df = year_2026_df[
            (year_2026_df['H100 equivalents'] >= min_size) & 
            (year_2026_df['H100 equivalents'] < max_size)
        ]
    ...
    # Calculate total compute for forecasted 2027
    total_compute_2027 = year_2027_df['H100 equivalents'].sum()
    ...
    for j, (min_size, max_size) in enumerate(size_categories):
        ...
        category_df = year_2027_df[
            (year_2027_df['H100 equivalents'] >= min_size) & 
            (year_2027_df['H100 equivalents'] < max_size)
        ]
    ...
    for i, year in enumerate(years):
        ...
        if len(year_df) > 0:
            ...
            cluster_sizes = year_df['H100 equivalents'].values
...
fig.text(0.5, 0.05, 'Year Cluster is Operational', ha='center', fontsize=12)

# Don't use tight_layout as it can interfere with our manual adjustments
# We've already set subplots_adjust earlier

# Save the figure
fig.savefig('new_compute_distribution_trend.png', dpi=300, bbox_inches='tight')
...
for year in years:
    ...
    if len(year_df) > 0:
        ...
        num_clusters = len(year_df)
        avg_size = total_compute / num_clusters
        print(f"{year}: {num_clusters} clusters, {total_compute:,.0f} H100 equivalents total, {avg_size:,.0f} average size")
    else:
